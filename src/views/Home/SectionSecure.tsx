...
export const SectionSecure: FC = () => {
  const i18n = useI18N();

  return (
    <section
      id="secure"
      className="relative isolate overflow-hidden bg-sky-50 bg-cover bg-center bg-no-repeat dark:bg-indigo-400/10"
    >
      <div className="absolute inset-x-0 top-[-10rem] -z-10 translate-y-80 translate-x-80 -rotate-45 transform-gpu overflow-hidden blur-3xl sm:top-[-20rem]">
        <svg
      ...
      <div className="relative mx-auto max-w-screen-lg px-4 py-16 sm:py-32 sm:px-6 lg:flex lg:h-screen lg:items-center lg:px-8">
        <div className="grid grid-cols-1 gap-y-8 lg:grid-cols-2 lg:items-end lg:gap-x-16">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            {i18n.content?.secureSection.items.map((item) => {
              return (
                <div
                  key={item.label}
          ...
          <div className="mx-auto max-w-lg text-center lg:mx-0 lg:text-right">
            <p className="mt-4 text-gray-600 dark:text-gray-400">
              {i18n.content?.secureSection.summary}
            </p>
            <h2 className="mt-4 text-3xl font-bold sm:text-4xl">
              {i18n.content?.secureSection.title}
            </h2>
          </div>
        </div>
      </div>
